# -*- coding: utf-8 -*-
"""
MeDeA 消融实验 - 无CNN版本 (w/o CNN Backbone)
使用线性投影层替代CNN特征提取器，测试CNN组件的重要性
对应表格: (F) Only Attention + Multi-Query
"""
import os
import argparse
import random
...
class LinearFeatureExtractor(nn.Module):
    """🔥 线性特征提取器 - 替代CNN，直接使用线性层处理原始信号"""
    def __init__(self, input_channels=12, d_model=256, dropout=0.3):
        super().__init__()
        
        # 直接将每个时间点的12导联信号映射到d_model维度
...
# 🔥 消融实验模型：无CNN版本
class MeDeANoCNN(nn.Module):
    """消融实验：移除CNN特征提取器的模型"""
    def __init__(self, num_classes=5, d_model=256, num_heads=8, dropout=0.3):
        super().__init__()
...
def run_single_fold_ablation(args, fold):
    ...
    for epoch in range(args.epochs):
        ...
        if val_f1 > best_f1:
            best_f1 = val_f1
            patience_counter = 0
            torch.save(model.state_dict(), f'best_no_cnn_fold_{fold}.pth')
        else:
            patience_counter += 1
    ...
    # 测试评估
    model.load_state_dict(torch.load(f'best_no_cnn_fold_{fold}.pth'))
    test_f1, test_auc = run_evaluation(model, test_loader, device)
    
    # 清理临时文件
    os.remove(f'best_no_cnn_fold_{fold}.pth')
    
    return test_f1, test_auc

def main():
    parser = argparse.ArgumentParser(description='MeDeA Ablation: No CNN Backbone')
    ...
    print(f"\n📊 消融实验结果 (w/o CNN Backbone):")
    print(f"Macro F1:  {mean_f1:.4f} ± {std_f1:.4f}")
    print(f"Macro AUC: {mean_auc:.4f} ± {std_auc:.4f}")
    
    # 保存结果
    results = {
        'experiment': 'w/o CNN Backbone',
        'description': 'Only Attention + Multi-Query',
        'mean_f1': mean_f1,
        'std_f1': std_f1,
        'mean_auc': mean_auc,
        'std_auc': std_auc,
    ...
    with open('ablation_no_cnn_results.json', 'w') as f:
